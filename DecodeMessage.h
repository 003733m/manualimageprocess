...
class DecodeMessage {
    ...
    std::string extractLSB(const ImageMatrix &image, const std::vector<std::pair<int, int>> &edgePixels);

    static int getLSB(double value);

    static std::string binaryToASCII(const std::string &binaryMessage);

    std::string circularShiftRight(const std::string &input, int shiftAmount);
};
